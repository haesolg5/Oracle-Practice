...
파일명 : Or09join.sql
테이블조인
설명 : 2개 이상의 테이블을 동시에 참조하여 데이터를 인출해야 할 때
    사용하는 SQL문
*/
...
검색할 때 사용한다.
-일반적으로 기본키(Primary key)와 외래키(Foreingn Key)를 사용하여 Join하는
경우가 대부분이다.
-2개의 테이블에 동일한 이름의 컬럼이 존재해야 하고ㅓ, 이 경우 '테이블명.컬럼명'
과 같이 기술한다.
...
from employees inner join departments
    on employees.department_id=departments.department_id;
/*
실행결과에서는 소속된 부서가 없는 1명을 베외한 나머지 106명의
레코드가 인출된다. 즉 inner join은 조인한 테이블 양쪽에 만조고디는
레코드를 대상으로 인출하게 된다. 
*/
...
--테이블의 별칭을 사용하면 쿼리문을 간단하게 만들 수 있다.
select
    first_name, last_name, email,
